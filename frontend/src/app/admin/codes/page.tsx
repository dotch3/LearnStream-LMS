...
export default function AdminCodesPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await load();
      showToast(editId ? 'Code updated' : 'Code created', 'success');
      resetForm();
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      title="Delete code?"
      description="This enrollment code will be permanently deleted and can no longer be redeemed."
      confirmLabel="Delete"
      variant="danger"
    ...
    <div className="max-w-5xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--ls-text)' }}>Enrollment Codes</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--ls-muted)' }}>Create codes that grant instant access to one or more courses</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="rounded-2xl p-6 mb-6" style={{ background: 'var(--ls-card)', border: '1px solid var(--ls-border)' }}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: 'var(--ls-muted)' }}>Label</label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: 'var(--ls-muted)' }}>Max Uses (optional)</label>
              <input
                type="number"
                className="w-full rounded-lg px-3 py-2 text-sm outline-none"
                style={{ background: 'var(--ls-bg)', border: '1px solid var(--ls-border)', color: 'var(--ls-text)' }}
            ...
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: 'var(--ls-muted)' }}>Expires At (optional)</label>
              <input
                type="date"
                className="w-full rounded-lg px-3 py-2 text-sm outline-none"
                style={{ background: 'var(--ls-bg)', border: '1px solid var(--ls-border)', color: 'var(--ls-text)' }}
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium" style={{ color: 'var(--ls-muted)' }}>
                Courses this code unlocks
              </label>
              {tracks.length > 0 && (
                <span className="text-xs" style={{ color: 'var(--ls-muted)' }}>
                  {form.trackIds.length} of {tracks.length} selected
                </span>
              )}
            </div>
            <div
            ...
            >
              {tracks.length === 0 ? (
                <p className="px-4 py-3 text-xs" style={{ color: 'var(--ls-muted)' }}>No courses available</p>
              ) : (
                tracks.map((t, i) => {
                  const sel = form.trackIds.includes(t.id);
                  return (
                    <label
                      key={t.id}
            ...
            <div className="mb-4">
              <label className="flex items-center gap-3 cursor-pointer w-fit">
                ...
                <span className="text-sm font-medium" style={{ color: 'var(--ls-text-1)' }}>
                  {form.isActive ? 'Active — users can redeem this code' : 'Inactive — code cannot be redeemed'}
                </span>
              </label>
            </div>
          )}
      ...
      {loading ? (
        ...
        <div className="rounded-xl py-16 text-center" style={{ background: 'var(--ls-card)', border: '1px solid var(--ls-border)' }}>
          <p className="text-base" style={{ color: 'var(--ls-muted)' }}>No enrollment codes yet</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: 'var(--ls-card)', borderBottom: '1px solid var(--ls-border)' }}>
                <th className="px-5 py-3 text-left font-semibold" style={{ color: 'var(--ls-muted)' }}>Code</th>
                <th className="px-5 py-3 text-left font-semibold" style={{ color: 'var(--ls-muted)' }}>Label</th>
                <th className="px-5 py-3 text-left font-semibold" style={{ color: 'var(--ls-muted)' }}>Courses</th>
                <th className="px-5 py-3 text-left font-semibold" style={{ color: 'var(--ls-muted)' }}>Uses</th>
                <th className="px-5 py-3 text-left font-semibold" style={{ color: 'var(--ls-muted)' }}>Expires</th>
