...
export default function AdminTrackEnrollmentsPage() {
  ...
  return (
    <>
      ...
      {/* Add student modal — bottom-sheet on mobile */}
      {showAdd && (
        <div
        ...
        >
          ...
          >
            ...
            {usersLoading ? (
              ...
              <div className="max-h-52 overflow-y-auto rounded-lg mb-3" style={{ border: '1px solid var(--ls-border)' }}>
                {filtered.map((u, i) => (
                  <button
                    key={u.id}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-6">
          ...
          <button
            onClick={openAdd}
            className="shrink-0 flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-90 cursor-pointer"
            style={{ background: 'var(--ls-accent)' }}
          >
        ...
        {/* Filter tabs */}
        <div
          className="flex gap-1 mb-5 p-1 rounded-lg overflow-x-auto tabs-scroll"
          style={{ background: 'var(--ls-surface)', border: '1px solid var(--ls-border)' }}
        >
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-14 rounded-xl animate-pulse" style={{ background: 'var(--ls-surface)' }} />
            ))}
          </div>
                ...
                return (
                  ...
                  >
                    <div className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium truncate" style={{ color: 'var(--ls-text-1)' }}>{e.userName}</p>
                          <p className="text-xs truncate" style={{ color: 'var(--ls-text-2)' }}>{e.userEmail}</p>
                        </div>
                        <span
                      ...
                      <p className="text-xs mt-2" style={{ color: 'var(--ls-text-3)' }}>
                        {new Date(e.requestedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </p>
                    </div>
                    ...
                    <div
                      className="grid"
                      style={{
                        gridTemplateColumns: e.status === 'PENDING' ? '1fr 1fr 1fr' : '1fr 1fr',
                      ...
                      {e.status === 'PENDING' && (
                        <>
                          <button
                            ...
                            className="flex items-center justify-center gap-1.5 py-3 text-sm font-medium cursor-pointer transition-opacity hover:opacity-70 disabled:opacity-40"
                          ...
                          <button
                            ...
                            disabled={acting === e.id}
                            className="flex items-center justify-center gap-1.5 py-3 text-sm font-medium cursor-pointer transition-opacity hover:opacity-70 disabled:opacity-40"
                            style={{ color: '#f59e0b', borderLeft: '1px solid var(--ls-border)' }}
                          >
                            <XIcon />
                            {t('deny')}
                          </button>
                          <button
                            ...
                            className="flex items-center justify-center gap-1.5 py-3 text-sm font-medium cursor-pointer transition-opacity hover:opacity-70"
          ...
          <div className="flex items-center justify-between mt-4 text-sm" style={{ color: 'var(--ls-text-2)' }}>
            ...
            <div className="flex gap-2">
              <button
                ...
                style={{ borderColor: 'var(--ls-border)', background: 'var(--ls-surface)', color: 'var(--ls-text-1)' }}
              >
                {tCommon('previous')}
              </button>
              <button
