  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--ls-bg)' }}>
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold" style={{ color: 'var(--ls-text)' }}>{t('title')}</h1>
          <p className="mt-1" style={{ color: 'var(--ls-muted)' }}>{t('subtitle')}</p>
        </div>
        ...
        {!loading && error && (
          <div className="rounded-xl p-6 text-center" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)' }}>
            ...
            <button onClick={() => load()} className="mt-2 text-sm underline" style={{ color: '#ef4444' }}>{tc('tryAgain')}</button>
          </div>
        )}
        ...
        {!loading && !error && tracks.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-xl py-20 text-center" style={{ border: '2px dashed var(--ls-border)', background: 'var(--ls-card)' }}>
            ...
            <h2 className="text-lg font-semibold" style={{ color: 'var(--ls-text)' }}>{t('empty')}</h2>
        ...
        {!loading && !error && tracks.length > 0 && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {tracks.map((track) => {
                const badgeStyle = STATUS_BADGE_STYLE[track.enrollmentStatus];
                return (
                  <div
                    key={track.id}
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {badgeStyle && (
                          ...
                          >
                            {t(`status.${track.enrollmentStatus}`)}
                          </span>
                        )}
                      </div>
                      {track.description && (
                      ...
                      <p className="text-xs mt-3" style={{ color: 'var(--ls-muted)' }}>
                        {t('videos', { count: track.videoCount })}
                      </p>
                    </div>
                  </div>
                );
